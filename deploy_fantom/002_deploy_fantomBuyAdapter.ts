...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("FantomBuyAdapter");
  ...
  const args = [config.WFTM];

  const result = await deploy("FantomBuyAdapter", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const buyadapter = await deployments.get("FantomBuyAdapter");
    ...
    if (networkName === "tenderly") {
      ...
      await hre.run("verify:verify", {
        ...
        contract: "contracts/adapters/fantom/FantomBuyAdapter.sol:FantomBuyAdapter",
      });
    }
  }
};
export default func;
func.tags = ["FantomBuyAdapter"];
