...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("FantomAdapter01");
  ...
  const args = [config.WFTM, config.AAVE_V3_REF_CODE, config.AAVE_V3_POOL, config.AAVE_V3_WETH_GATEWAY];
  const networkName = hre.network.name;

  const result = await deploy("FantomAdapter01", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    ...
    const adapter01 = await deployments.get("FantomAdapter01");
