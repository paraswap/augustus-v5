...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("FantomAdapter02");
  ...
  const result = await deploy("FantomAdapter02", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    ...
    const adapter02 = await deployments.get("FantomAdapter02");
