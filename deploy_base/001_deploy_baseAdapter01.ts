...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("BaseAdapter01");
  ...
  const args = [config.WETH, config.AAVE_V3_REF_CODE, config.AAVE_V3_POOL, config.AAVE_V3_WETH_GATEWAY];
  const networkName = hre.network.name;

  const result = await deploy("BaseAdapter01", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    ...
    const adapter01 = await deployments.get("BaseAdapter01");
    ...
    if (networkName === "tenderly") {
      ...
      await hre.run("verify:verify", {
        ...
        contract: "contracts/adapters/base/BaseAdapter01.sol:BaseAdapter01",
      });
    }
  }
};
export default func;
func.tags = ["BaseAdapter01"];
