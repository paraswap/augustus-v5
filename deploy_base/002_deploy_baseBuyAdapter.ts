...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("BaseBuyAdapter");
  ...
  const result = await deploy("BaseBuyAdapter", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const buyadapter = await deployments.get("BaseBuyAdapter");
    ...
    if (networkName === "tenderly") {
      ...
      await hre.run("verify:verify", {
        ...
        contract: "contracts/adapters/base/BaseBuyAdapter.sol:BaseBuyAdapter",
      });
    }
  }
};
export default func;
func.tags = ["BaseBuyAdapter"];
