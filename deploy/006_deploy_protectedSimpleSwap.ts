...
const func: DeployFunction = async ({
  ...
  const artifact = await deployments.getArtifact("ProtectedSimpleSwap");
  ...
  const result = await deploy("ProtectedSimpleSwap", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const simpleSwap = await deployments.get("ProtectedSimpleSwap");
    const simpleSwapInstance = await ethers.getContractAt("ProtectedSimpleSwap", simpleSwap.address);
    ...
    tx = await augustusAdmin.setImplementation(
      simpleSwapInstance.interface.getSighash("protectedSimpleSwap"),
      simpleSwap.address,
    );
    await tx.wait();
    tx = await augustusAdmin.setImplementation(
      simpleSwapInstance.interface.getSighash("protectedSimpleBuy"),
      simpleSwap.address,
    );
    await tx.wait();
