...
const func: DeployFunction = async ({
  ...
  const artifact = await deployments.getArtifact("OnERC721Received");
  ...
  const result = await deploy("OnERC721Received", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const erc721Receiver = await deployments.get("OnERC721Received");
    const erc721ReceiverInstance = await ethers.getContractAt(artifact.abi, erc721Receiver.address);
    ...
    let tx = await augustusAdmin.grantRole(role, erc721Receiver.address);
    await tx.wait();
    tx = await augustusAdmin.setImplementation(
      erc721ReceiverInstance.interface.getSighash("onERC721Received"),
    ...
    if (networkName === "tenderly") {
      await hre.tenderly.verify({
        ...
        address: erc721Receiver.address,
      });
