...
const func: DeployFunction = async ({
  ...
  const artifact = await deployments.getArtifact("SimpleSwapNFT");
  ...
  const result = await deploy("SimpleSwapNFT", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const simpleSwapNFT = await deployments.get("SimpleSwapNFT");
    const simpleSwapNFTInstance = await ethers.getContractAt(artifact.abi, simpleSwapNFT.address);
    const role = "0x7a05a596cb0ce7fdea8a1e1ec73be300bdb35097c944ce1897202f7a13122eb2";
    let tx = await augustusAdmin.grantRole(role, simpleSwapNFT.address);
    await tx.wait();
    tx = await augustusAdmin.setImplementation(
      simpleSwapNFTInstance.interface.getSighash("simpleBuyNFT"),
      simpleSwapNFT.address,
    );
    await tx.wait();
