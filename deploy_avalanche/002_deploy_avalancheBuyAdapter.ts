...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("AvalancheBuyAdapter");
  ...
  const args = [config.WAVAX];

  const result = await deploy("AvalancheBuyAdapter", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const buyadapter = await deployments.get("AvalancheBuyAdapter");
    ...
    if (networkName === "tenderly") {
      ...
      await hre.run("verify:verify", {
        ...
        contract: "contracts/adapters/avalanche/AvalancheBuyAdapter.sol:AvalancheBuyAdapter",
      });
    }
  }
};
export default func;
func.tags = ["AvalancheBuyAdapter"];
