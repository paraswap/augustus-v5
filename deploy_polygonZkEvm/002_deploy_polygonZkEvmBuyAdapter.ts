...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("PolygonZkEvmBuyAdapter");
  ...
  const result = await deploy("PolygonZkEvmBuyAdapter", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const buyadapter = await deployments.get("PolygonZkEvmBuyAdapter");
    ...
    if (networkName === "tenderly") {
      ...
      await hre.run("verify:verify", {
        ...
        contract: "contracts/adapters/polygonZkEvm/PolygonZkEvmBuyAdapter.sol:PolygonZkEvmBuyAdapter",
      });
    }
  }
};
export default func;
func.tags = ["PolygonZkEvmBuyAdapter"];
