...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("RopstenBuyAdapter");
  ...
  const result = await deploy("RopstenBuyAdapter", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const buyadapter = await deployments.get("RopstenBuyAdapter");
    ...
    if (networkName === "tenderly") {
      ...
      await hre.run("verify:verify", {
        ...
        contract: "contracts/adapters/ropsten/RopstenBuyAdapter.sol:RopstenBuyAdapter",
      });
    }
  }
};
export default func;
func.tags = ["RopstenBuyAdapter"];
