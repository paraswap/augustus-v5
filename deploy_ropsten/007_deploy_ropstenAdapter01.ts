...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("RopstenAdapter01");
  ...
  const args = [config.UNISWAPV1_FACTORY, config.WETH];
  const networkName = hre.network.name;

  const result = await deploy("RopstenAdapter01", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    ...
    const adapter01 = await deployments.get("RopstenAdapter01");
