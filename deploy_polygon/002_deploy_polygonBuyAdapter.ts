...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("PolygonBuyAdapter");
  ...
  const args = [config.WMATIC];

  const result = await deploy("PolygonBuyAdapter", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const buyadapter = await deployments.get("PolygonBuyAdapter");
    ...
    if (networkName === "tenderly") {
      ...
      await hre.run("verify:verify", {
        ...
        contract: "contracts/adapters/polygon/PolygonBuyAdapter.sol:PolygonBuyAdapter",
      });
    }
  }
};
export default func;
func.tags = ["PolygonBuyAdapter"];
