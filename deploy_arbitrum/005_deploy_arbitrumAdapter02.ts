...
const func: DeployFunction = async ({
  ...
  const artifact = await deployments.getArtifact("ArbitrumAdapter02");
  ...
  const args: Array<string | number> = [config.WETH];

  const result = await deploy("ArbitrumAdapter02", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const adapter02 = await deployments.get("ArbitrumAdapter02");
