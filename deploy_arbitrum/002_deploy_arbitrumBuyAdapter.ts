...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("ArbitrumBuyAdapter");
  ...
  const result = await deploy("ArbitrumBuyAdapter", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const buyadapter = await deployments.get("ArbitrumBuyAdapter");
    ...
    if (networkName === "tenderly") {
      ...
      await hre.run("verify:verify", {
        ...
        contract: "contracts/adapters/arbitrum/ArbitrumBuyAdapter.sol:ArbitrumBuyAdapter",
      });
    }
  }
};
export default func;
func.tags = ["ArbitrumBuyAdapter"];
