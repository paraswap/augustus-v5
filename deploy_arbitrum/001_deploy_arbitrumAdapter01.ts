...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("ArbitrumAdapter01");
  ...
  const result = await deploy("ArbitrumAdapter01", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    ...
    const adapter01 = await deployments.get("ArbitrumAdapter01");
