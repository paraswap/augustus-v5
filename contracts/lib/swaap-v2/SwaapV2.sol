...
contract SwaapV2 {
    ...
    function swapOnSwaapV2(
        ...
        if (isBatchSwap) {
            (, uint256[] memory callerSlots, BalancerBatchData memory data) = abi.decode(
                payload,
                (bool, uint256[], BalancerBatchData)
            );
            ...
            (, uint256 callerSlot, BalancerSimpleData memory data) = abi.decode(
                payload,
                (bool, uint256, BalancerSimpleData)
            );
            ...
            if (address(fromToken) == Utils.ethAddress()) {
                ...
                Utils.approve(vault, address(fromToken), fromAmount);
                IBalancerV2Vault(vault).swap(data.singleSwap, data.funds, data.limit, block.timestamp);
            }
        }
    }
