...
abstract contract UniswapV2 is WethProvider {
    ...
    function swapOnUniswap(
        ...
        _swapOnUniswapV2(fromAmount, data.path);
    }
    ...
    function buyOnUniswap(
        ...
        _buyOnUniswapV2(fromAmount, toAmount, fee, feeFactor, factory, initCode, data.path);
    }
    ...
    function swapOnUniswapFork(
        ...
        _swapOnUniswapV2Fork(fromAmount, data.path, data);
    }
    ...
    function buyOnUniswapFork(
        ...
        _buyOnUniswapV2(fromAmount, toAmount, data.fee, data.feeFactor, data.factory, data.initCode, data.path);
    }
        ...
        for (uint8 i = 0; i < length - 1; i++) {
            ...
            if (i == 0) {
                if (tokenSold == Utils.ethAddress()) {
                    ...
                    TransferHelper.safeTransfer(tokenSold, pairs[i], tokensSold);
                }
            }
            ...
            IUniswapV2Pair(pairs[i]).swap(amount0Out, amount1Out, receiver, new bytes(0));
        }
        if (tokensBoughtEth) {
        ...
        for (uint8 i = 0; i < pairs; i++) {
            ...
            if (i == 0) {
                if (tokenSold == Utils.ethAddress()) {
                    ...
                    currentPair = UniswapV2Lib.pairFor(data.factory, tokenSold, tokenBought, data.initCode);
                    TransferHelper.safeTransfer(tokenSold, currentPair, fromAmount);
                }
            }
            ...
            if ((i + 1) == pairs) {
                ...
                receiver = UniswapV2Lib.pairFor(
                    data.factory,
                    tokenBought,
                    path[i + 2] == Utils.ethAddress() ? WETH : path[i + 2],
                    data.initCode
                );
            }
    ...
    function _swapOnUniswapV2(uint256 fromAmount, address[] memory path) private returns (uint256 tokensBought) {
        require(path.length > 1, "More than 1 token required");
        uint8 pairs = uint8(path.length - 1);
        bool tokensBoughtEth;
        tokensBought = fromAmount;
