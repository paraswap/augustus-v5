...
const func: DeployFunction = async ({
  ...
  const artifact = await deployments.getArtifact("BscBuyAdapter");
  ...
  const args = [config.WBNB];

  const result = await deploy("BscBuyAdapter", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const buyadapter = await deployments.get("BscBuyAdapter");
    ...
    if (networkName === "tenderly") {
      ...
      await hre.run("verify:verify", {
        ...
        contract: "contracts/adapters/bsc/BscBuyAdapter.sol:BscBuyAdapter",
      });
    }
  }
};
export default func;
func.tags = ["BscBuyAdapter"];
