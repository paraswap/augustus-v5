...
const func: DeployFunction = async ({
  ...
  const artifact = await deployments.getArtifact("BuyAdapter02");
  ...
  const result = await deploy("BuyAdapter02", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    const buyAdapter02 = await deployments.get("BuyAdapter02");
    const role = await augustusInstance.WHITELISTED_ROLE();
    const tx = await augustusAdmin.grantRole(role, buyAdapter02.address);
    await tx.wait();

    if (networkName === "tenderly") {
      await hre.tenderly.verify({
        name: "BuyAdapter",
        address: buyAdapter02.address,
      });
      ...
      await hre.run("verify:verify", {
        ...
        contract: "contracts/adapters/mainnet/BuyAdapter02.sol:BuyAdapter02",
      });
    }
  }
};
export default func;
func.tags = ["BuyAdapter02"];
