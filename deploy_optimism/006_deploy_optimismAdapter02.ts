...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("OptimismAdapter02");
  ...
  const result = await deploy("OptimismAdapter02", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    ...
    const adapter02 = await deployments.get("OptimismAdapter02");
    ...
    if (networkName === "tenderly") {
      ...
      await hre.run("verify:verify", {
        address: adapter02.address,
        contract: "contracts/adapters/optimism/OptimismAdapter02.sol:OptimismAdapter02",
