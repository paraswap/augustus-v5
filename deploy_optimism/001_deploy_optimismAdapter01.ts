...
const func: DeployFunction = async ({ getNamedAccounts, deployments, getChainId }: HardhatRuntimeEnvironment) => {
  ...
  const artifact = await deployments.getArtifact("OptimismAdapter01");
  ...
  const result = await deploy("OptimismAdapter01", {
    from: deployer,
    contract: {
      abi: artifact.abi,
  ...
  if (result.newlyDeployed) {
    ...
    const adapter01 = await deployments.get("OptimismAdapter01");
